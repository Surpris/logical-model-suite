...
export class PrismaSchemaBuilder {
    ...
    for (const [entityName, entity] of Object.entries(entities)) {
      ...
      for (const [relName, rel] of Object.entries(entity.relationships)) {
        ...
        if (rel.cardinality === "1:N") {
          ...
          const backRefName =
            this.toSafeCamelCase(relName) + this.toSafePascalCase(entityName);
          ...
          const inverseName =
            this.toSafeCamelCase(relName) +
            this.toSafePascalCase(entityName) +
            "s";
  ...
  private mapType(
    ...
    switch (yamlType) {
      ...
      case "Text":
        return "String";
      case "Enum":
        return `${modelName}${this.toPascalCase(attrName)}`;
      default:
        return "String";
    }
  }
  ...
  private toSafePascalCase(str: string): string {
    ...
    if (
      this.reservedWords.has(pascal) ||
      this.reservedWords.has(pascal.toLowerCase())
    ) {
      return `${pascal}_`;
    }
    return pascal;
  ...
  private toSafeCamelCase(str: string): string {
    ...
    if (
      this.reservedWords.has(camel) ||
      this.reservedWords.has(camel.toLowerCase())
    ) {
      return `${camel}_`;
    }
    return camel;
